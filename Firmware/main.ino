/*
  E-inkAware — WeAct 2.9" B/W (GxEPD2_BW)
...
// FIX: core 3.x uses pin-based LEDC, no channel needed
...
const uint32_t INACTIVITY_MIN = 30;
...
const unsigned long BLE_ENABLE_TIMEOUT_MS = 2UL * 60UL * 1000UL;

// WiFi / OTA
bool          wifiConnected = false;
bool          otaActive     = false;
unsigned long otaStartTs    = 0;
const unsigned long OTA_ENABLE_TIMEOUT_MS = 20UL * 60UL * 1000UL;
...
// Encoder
static portMUX_TYPE encoderMux = portMUX_INITIALIZER_UNLOCKED;  // ADD THIS
...
void ensureFilesystems() {
  if (!filesystemReady) filesystemReady = LittleFS.begin(true);
...
void refreshNotesIndex() {
  ...
  for (File file = root.openNextFile(); file && noteCount < MAX_NOTES; file = root.openNextFile()) {
    ...
    if (!path.startsWith("/")) path = String(NOTES_DIR) + "/" + path;
  ...
  if (noteCount == 0) { selectedNoteIndex = 0; noteViewIndex = 0; notesPageOffset = 0; noteScrollLine = 0; return; }
...
int    loadNotesCount() { return noteCount; }
String readNote(int idx) { if (idx < 0 || idx >= noteCount) return ""; return readFileText(noteFiles[idx]); }
...
void clearAllNotes() {
  if (!filesystemReady) return;
  for (int i = 0; i < noteCount; ++i) LittleFS.remove(noteFiles[i]);
  refreshNotesIndex();
}
...
bool addNote(const String &text) {
  ...
  if (noteCount >= MAX_NOTES) { LittleFS.remove(noteFiles[0]); refreshNotesIndex(); }
...
bool syncTimeFromNTP() {
  if (!connectWiFiForServices()) return false;
  configTime(0, 0, "pool.ntp.org", "time.nist.gov");
  struct tm timeinfo;
  bool ok = getLocalTime(&timeinfo, 5000);
  if (!otaActive) disconnectWiFi();
  return ok;
}
...
void setFrontLight(bool on) {
  ...
  frontLightOn = on;
  // FIX: core 3.x uses ledcWrite(pin, duty) not ledcWrite(channel, duty)
  ledcWrite(LED_PWM_PIN, on ? LIGHT_PWM_ON : LIGHT_PWM_MIN);
}

void toggleLightMode() {
  switch (lightMode) {
    case LIGHT_MODE_AUTO:       setLightMode(LIGHT_MODE_FORCED_ON);  setFrontLight(true);  break;
    case LIGHT_MODE_FORCED_ON:  setLightMode(LIGHT_MODE_FORCED_OFF); setFrontLight(false); break;
    default:                    setLightMode(LIGHT_MODE_AUTO); break;
  }
}
...
void autoAdjustLight() {
  ...
  switch (lightMode) {
    case LIGHT_MODE_FORCED_ON:  setFrontLight(true);  return;
    case LIGHT_MODE_FORCED_OFF: setFrontLight(false); return;
    default: break;
  }
  float lux = lightMeter.readLightLevel();
  if (lux < 0) return; 
  if (!frontLightOn && lux < LUX_THRESHOLD_ON)   setFrontLight(true);
  else if (frontLightOn && lux > LUX_THRESHOLD_OFF) setFrontLight(false);
}

// ========== PAGE ROUTING ==========
PageMode cycleMainPage(PageMode page, int step) {
  const PageMode cycle[] = {PAGE_DASHBOARD, PAGE_NOTES, PAGE_SYSTEM, PAGE_OTA};
  int index = 0;
  for (int i = 0; i < 4; i++) { if (cycle[i] == page) { index = i; break; } }
...
void drawDashboard() {
  ...
  do {
    ...
    drawFrame("Dashboard", bleEnabled ? "BLE on" : "");
    display.setFont(&FreeMonoBold9pt7b);
    display.setTextColor(GxEPD_BLACK);
    ...
    display.setTextSize(1);
    display.setCursor(168, 34);
    String dateLabel = currentDateLabel();
    display.print(dateLabel.length() > 0 ? dateLabel : "No date");
    display.setFont(nullptr);
    display.setTextSize(1);
    ...
    display.printf("Notes: %d  Light: %s", loadNotesCount(), currentLightModeLabel().c_str());
    ...
    drawFooter("Short:light  Long:BLE  Turn:pages");
...
void drawNotesList() {
  ...
  do {
    ...
    if (noteCount == 0) {
      ...
      display.print("Send NOTE:... over BLE.");
      drawFooter("Long:BLE  Turn:pages");
    } else {
      if (notesPageOffset > selectedNoteIndex)
        notesPageOffset = (selectedNoteIndex / NOTES_PER_PAGE) * NOTES_PER_PAGE;
      ...
      for (int i = notesPageOffset; i < endIndex; ++i) {
        ...
        int y    = 32 + row * 14;
        bool sel = (i == selectedNoteIndex);
        if (sel) { display.fillRect(6, y - 10, 284, 13, GxEPD_BLACK); display.setTextColor(GxEPD_WHITE); }
      ...
      drawFooter("Short:open  Long:BLE  Pg " + String(pageNumber) + "/" + String(totalPages));
    }
  } while (display.nextPage());
  display.hibernate();
}
...
void drawNote(int idx) {
  ...
  int totalLines = max(1, ((int)text.length() + NOTE_CHARS_PER_LINE - 1) / NOTE_CHARS_PER_LINE);
  ...
  do {
    display.fillScreen(GxEPD_WHITE);
    drawFrame("Note " + String(idx + 1), String(noteScrollLine + 1) + "/" + String(totalLines));
    display.setFont(nullptr);
    display.setTextSize(1);
    display.setTextColor(GxEPD_BLACK);
    int lineY = 32;
    ...
    drawFooter("Short:back  Long:delete  Turn:scroll");
  } while (display.nextPage());
  display.hibernate();
}
...
void drawSystem() {
  ...
  do {
    ...
    display.setTextSize(1);
    display.setTextColor(GxEPD_BLACK);
    display.setCursor(8, 30);  display.printf("Notes: %d", loadNotesCount());
    display.setCursor(8, 42);  display.printf("BLE: %s  OTA: %s", bleEnabled ? "on" : "off", otaActive ? "on" : "off");
    display.setCursor(8, 54);  display.printf("WiFi: %s  Light: %s", wifiConnected ? "on" : "off", currentLightModeLabel().c_str());
    display.setCursor(8, 66);  display.printf("Heap: %u", (unsigned)ESP.getFreeHeap());
    display.setCursor(8, 78);  display.printf("FS: %s", filesystemReady ? "ready" : "error");
    if (filesystemReady) {
      display.setCursor(8, 90);
      display.printf("Storage: %s / %s",
        bytesLabel(LittleFS.usedBytes()).c_str(),
        bytesLabel(LittleFS.totalBytes()).c_str());
    }
    drawFooter("Short:OTA  Long:BLE  Turn:pages");
  } while (display.nextPage());
  display.hibernate();
...
void drawOtaPage() {
  ...
  do {
    ...
    if (otaActive && WiFi.status() == WL_CONNECTED) {
      ...
      display.print("Need WiFi creds to use OTA.");
    }
    display.setCursor(8, 42); display.print("Hostname: "); display.print(DEVICE_NAME);
    display.setCursor(8, 54); display.print(otaActive ? "ArduinoOTA is live" : "BLE: WIFI:ssid|pass");
    display.setCursor(8, 66); display.print("Short: start/stop OTA");
    display.setCursor(8, 78); display.print("Keep device awake during upload.");
    drawFooter("Short:toggle  Long:BLE  Turn:pages");
  } while (display.nextPage());
  display.hibernate();
...
void setupOtaHandlers() {
  ...
  ArduinoOTA.onStart([]() { setFrontLight(false); display.hibernate(); });
...
bool startOtaSession() {
  ...
  if (bleEnabled) bleStop();
  if (!connectWiFiForServices()) { drawOtaPage(); return false; }
  WiFi.setSleep(false); 
  setupOtaHandlers();
  ArduinoOTA.begin();
  otaActive   = true;
...
class TextCallback : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *characteristic) override {
    // FIX: core 3.x BLE getValue() returns String, not std::string
    String payload = characteristic->getValue();
    if (payload.length() == 0) return;
    String command = normalizeNoteText(payload);
    bool redraw = false;

    if (command.startsWith("NOTE:"))                           { addNote(command.substring(5)); redraw = true; }
    else if (command.startsWith("DEL:"))                       { int idx = command.substring(4).toInt(); if (idx > 0) { deleteNote(idx - 1); redraw = true; } }
    else if (command.startsWith("OPEN:") || command.startsWith("VIEW:")) {
      int idx = command.substring(5).toInt();
      if (idx > 0 && idx <= noteCount) { noteViewIndex = idx - 1; noteScrollLine = 0; currentPage = PAGE_NOTE_VIEW; redraw = true; }
    }
    else if (command == "CLEAR_NOTES")                         { clearAllNotes(); redraw = true; }
    else if (command.startsWith("WIFI:") && command != "WIFI:OFF") {
      String value = command.substring(5);
      int sep = value.indexOf('|');
      String ssid = sep >= 0 ? value.substring(0, sep) : value;
      String pass = sep >= 0 ? value.substring(sep + 1) : "";
      ssid.trim(); pass.trim();
      storeWiFiCredentials(ssid, pass); redraw = true;
    }
    else if (command == "WIFI:OFF")                            { storeWiFiCredentials("", ""); redraw = true; }
    else if (command == "TIME:SYNC")                           { syncTimeFromNTP(); redraw = true; }
    else if (command.startsWith("TIME:"))                      { unsigned long epoch = command.substring(5).toInt(); struct timeval tv = { (time_t)epoch, 0 }; settimeofday(&tv, nullptr); redraw = true; }
    else if (command == "LIGHT:AUTO")                          { setLightMode(LIGHT_MODE_AUTO); redraw = true; }
    else if (command == "LIGHT:ON")                            { setLightMode(LIGHT_MODE_FORCED_ON); setFrontLight(true); redraw = true; }
    else if (command == "LIGHT:OFF")                           { setLightMode(LIGHT_MODE_FORCED_OFF); setFrontLight(false); redraw = true; }
    else if (command == "OTA:START")                           { redraw = startOtaSession(); }
    else if (command == "OTA:STOP")                            { stopOtaSession(); redraw = true; }
    else if (command == "BLE:OFF")                             { bleStop(); return; }
    else                                                       { addNote(command); redraw = true; }

    lastActivityMs = millis();
    if (redraw) drawCurrentPage();
  }
};
...
portENTER_CRITICAL_ISR(&encoderMux);          // ADD
  ...
  if (transition == 0b1101 || transition == 0b0100 ||
      transition == 0b0010 || transition == 0b1011) {
    encoderDelta = encoderDelta + 1;
  } else if (transition == 0b1110 || transition == 0b0111 ||
             transition == 0b0001 || transition == 0b1000) {
    encoderDelta = encoderDelta - 1;
  }
  encoderLastState = state;
  portEXIT_CRITICAL_ISR(&encoderMux);           // ADD
}
...
void ensureSelectedNoteVisible() {
  if (noteCount == 0) { selectedNoteIndex = 0; notesPageOffset = 0; return; }
  if (selectedNoteIndex < 0)          selectedNoteIndex = 0;
  if (selectedNoteIndex >= noteCount) selectedNoteIndex = noteCount - 1;
...
void handleEncoderStep(int step) {
  if (otaActive || step == 0) return;
  if (currentPage == PAGE_NOTES) {
    if (noteCount == 0) { currentPage = cycleMainPage(currentPage, step); drawCurrentPage(); return; }
  ...
  if (currentPage == PAGE_NOTE_VIEW) {
    ...
    int totalLines = max(1, ((int)noteText.length() + NOTE_CHARS_PER_LINE - 1) / NOTE_CHARS_PER_LINE);
    noteScrollLine = constrain(noteScrollLine + step, 0, max(0, totalLines - NOTE_VISIBLE_LINES));
    drawNote(noteViewIndex);
    return;
  }
  currentPage = cycleMainPage(currentPage, step);
  drawCurrentPage();
}
...
void handleShortPress() {
  ...
  switch (currentPage) {
    case PAGE_DASHBOARD:
      ...
      break;
    case PAGE_NOTES:
      if (noteCount > 0) { noteViewIndex = selectedNoteIndex; noteScrollLine = 0; currentPage = PAGE_NOTE_VIEW; drawNote(noteViewIndex); }
      break;
    case PAGE_NOTE_VIEW:
    ...
    case PAGE_SYSTEM:
      ...
      break;
    case PAGE_OTA:
      // FIX: ternary with mismatched return types (bool vs void) — use if/else
      if (otaActive) stopOtaSession(); else startOtaSession();
      break;
  }
}
...
void handleLongPress() {
  ...
  if (bleEnabled) bleStop(); else bleStart();
}

// ========== LIGHT / POWER ==========
void setupLight() {
  // FIX: core 3.x replaced ledcSetup+ledcAttachPin with a single ledcAttach call
  ledcAttach(LED_PWM_PIN, LEDC_FREQ, LEDC_RES);
...
void goToDeepSleep() {
  setFrontLight(false);
  if (bleEnabled) bleStop();
  if (otaActive)  stopOtaSession();
  delay(20);
  esp_sleep_enable_ext0_wakeup((gpio_num_t)ENC_BTN_PIN, 0);
  Serial.println("Deep sleep...");
  prefs.end();
  display.hibernate();
  gpio_set_direction((gpio_num_t)LED_PWM_PIN, GPIO_MODE_OUTPUT);
  gpio_set_level((gpio_num_t)LED_PWM_PIN, 0);
  gpio_set_direction((gpio_num_t)LED_PWM_PIN, GPIO_MODE_OUTPUT);
  gpio_set_level((gpio_num_t)LED_PWM_PIN, 0);
...
void setup() {
  ...
  String ssid, pass;
  if (loadWiFiCredentials(ssid, pass)) syncTimeFromNTP();
...
void loop() {
  ...
  if (delta != 0) {
    encoderAccum += delta;
    while (encoderAccum >= 4)  { handleEncoderStep(1);  encoderAccum -= 4; lastActivityMs = millis(); }
    while (encoderAccum <= -4) { handleEncoderStep(-1); encoderAccum += 4; lastActivityMs = millis(); }
  }
  ...
  if (btn == LOW && !btnDown)   { btnDown = true; btnDownTs = millis(); }
  else if (btn == HIGH && btnDown) {
    btnDown = false;
    unsigned long held = millis() - btnDownTs;
    if (held >= 50 && held < 800) handleShortPress();
    else if (held >= 800)         handleLongPress();
  }

  if (bleEnabled && (millis() - bleEnableTs > BLE_ENABLE_TIMEOUT_MS)) bleStop();

  if (otaActive) {
    ArduinoOTA.handle();
    if (millis() - otaStartTs > OTA_ENABLE_TIMEOUT_MS) stopOtaSession();
  }

  autoAdjustLight();

  if (currentPage == PAGE_DASHBOARD && millis() - lastDashboardRefreshMs > 60000UL) drawDashboard();
  if (currentPage == PAGE_SYSTEM    && millis() - lastSystemRefreshMs    > 60000UL) drawSystem();
  if (currentPage == PAGE_OTA       && millis() - lastOtaRefreshMs       > 30000UL) drawOtaPage();

  if (!bleEnabled && !otaActive) {
    if (millis() - lastActivityMs > INACTIVITY_MIN * 60UL * 1000UL) {
      drawDashboard();
      delay(200);
      goToDeepSleep();
    }
